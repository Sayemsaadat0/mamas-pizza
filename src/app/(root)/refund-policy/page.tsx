...
export default function RefundPolicyPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 md:px-6 lg:px-8 py-10 mt-[200px]">
      <h1 className="text-2xl font-semibold mb-3">Refund Policy</h1>
      {restaurant?.refund_process ? (
        <div 
          className="text-foreground/70 prose prose-sm max-w-none"
          dangerouslySetInnerHTML={{ __html: restaurant.refund_process }}
        />
      ) : (
        <p className="text-foreground/70">No refund policy content available at the moment.</p>
      )}
    </main>
  );
}